...
contract MarketFactory is ERC1155, CloneFactory {
    ...
    function createItem (
        ...
        bool isEncrypt,
        address user
    ) external reEntrancyGurd isBlackList returns(uint) {
          ...
          if(_type == NFTtype.PRIVATE) {
            ...
            _privateMetadata[id].set = true;
             ...
             for(uint i; i < amount; i++){
               id = mintNFT(_uri, user, 1, _type, _royaltyFee, _tier0);
             }
        }
        return id;
    }

    function mintNFT(string memory _uri, address taker, uint amount, NFTtype _type, uint8 royaltyFee,uint _tier0) public  returns(uint id){
        _tokenIds.increment();
        uint newid = _tokenIds.current();
        _mint(taker, newid, amount, "");
        _tokenURIs[newid] = _uri;
        userTokenInfo[taker].push(newid);
        userInfo[newid].royaltyFee = royaltyFee;
        userInfo[newid].royaltyShare = 50;
        userInfo[newid].user = taker;
        userInfo[newid].nftType = _type;
        if(_type == NFTtype.FNFT) {
    ...
    function safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) public reEntrancyGurd virtual override {
    ...
    function safeBatchTransferFrom(
        ...
        for(uint j = 0; j < ids.length; j++) {
            uint id = ids[j];
            if((balanceOf(from, id) - amounts[j])== 0) {
                uint len = userTokenInfo[from].length;
                for(uint i = 0; i < len; i++) {
                    if(userTokenInfo[from][i] == id) {
                        userTokenInfo[from][i] = userTokenInfo[from][len-1];
