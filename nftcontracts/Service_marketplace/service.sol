...
contract ServiceMarket is Ownable, CloneFactory {
    ...
    modifier ReentrancyGurd() {
        require(isEntered == false, "Snft: reentrancy gurd");
        isEntered = true;
        _;
        isEntered = false;
    }
    ...
    function sNFTPutOnSale(address collectionId, uint tokenId, address coin, uint[] memory price) external payable isBlackList ReentrancyGurd{
        ...
        require(sNftInfo[_key].isAlive == false, "Snft: already listed");
    ...
    function confirmSNFT(bytes32 _key, address user) external isBlackList ReentrancyGurd{
        require(sNftInfo[_key].confirmUser == address(0), "SMain:already assigned");
        require(msg.sender == sNftInfo[_key].maker, "SMain:Not maker");
    ...
    function takeMilestone(bytes32 _key) external isBlackList ReentrancyGurd{
        require(msg.sender == sNftInfo[_key].confirmUser && !Dispute[_key], "SMain:not taker");
    ...
    function withdrawTokens(address coin, address user, uint amount) external onlyOwner ReentrancyGurd{
